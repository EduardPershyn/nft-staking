...
describe('Staking Game', function () {
    ...
    beforeEach(async () => {
        ...
        const erc721StakingFactory = await ethers.getContractFactory("Erc721_staking");
        ...
        const tx = await erc20Reward.connect(accounts[DEPLOYER_ID]).setGameManager(game.address);
        await tx.wait();
    });
    ...
    describe('erc20', function () {
        it('Reject mint for no owner', async () => {
            await expect(
                    erc20Reward.connect(accounts[PLAYER_ID1])
                                 .mint(1)
    ...
    describe('erc721', function () {
        it('Reject low fee', async () => {
            await expect(
                    erc721Staking.connect(accounts[PLAYER_ID1])
        ...
        it('Owner should be changed', async () => {
            await erc721Staking.connect(accounts[PLAYER_ID1]).
                mint({value: ethers.utils.parseEther("1")});

            const address = await erc721Staking.connect(accounts[PLAYER_ID1]).ownerOf(tokenId0);
            expect(address).to.be.equal(accounts[PLAYER_ID1].address);
        })

        it('Withdraw test', async () => {
            const balanceBefore = new BigNumber.from(await ethers.provider.getBalance(accounts[DEPLOYER_ID].address));
           ...
           const balanceAfter = new BigNumber.from(await ethers.provider.getBalance(accounts[DEPLOYER_ID].address));
           const balanceSub = balanceAfter.sub(balanceBefore);
           expect(balanceSub).to.be.closeTo(new BigNumber.from(
    ...
    describe('Game', function () {
        beforeEach(async () => {
            ...
            await erc721Staking.connect(accounts[PLAYER_ID1])
                ["safeTransferFrom(address,address,uint256)"](accounts[PLAYER_ID1].address, game.address, tokenId0);
        })

        it('Reject claim for no owner', async () => {
            await expect(
                    game.connect(accounts[PLAYER_ID2])
                                 .claimReward(tokenId0)
            ).to.be.revertedWith("Game: Only original owner can claim reward");
        })

        it('Withdraw', async () => {
            expect(await erc721Staking.connect(accounts[PLAYER_ID1]).ownerOf(tokenId0))
    ...
    describe('Wrong NFT Stake', function () {
        it('Reject if other NFT staked', async () => {
            ...
            await otherErc721.connect(accounts[PLAYER_ID1]).mint({value: ethers.utils.parseEther("1")});
            await expect(
                otherErc721.connect(accounts[PLAYER_ID1])
                    ["safeTransferFrom(address,address,uint256)"](accounts[PLAYER_ID1].address, game.address, tokenId0)
    ...
    describe('Stake and Claim balances', function () {
        ...
        it('Balance Checks', async () => {
            ...
            await time.increase( (DEFAULT_STAKE_PERIOD + (DEFAULT_STAKE_PERIOD/2)) * 60 * 60 ); //36 hours
            await game.connect(accounts[PLAYER_ID1]).claimReward(tokenId0);

            await checkBalance(new BigNumber.from(ethers.utils.parseEther(CLAIM_REWARD.toString())));

            await time.increase( DEFAULT_STAKE_PERIOD/2 * 60 * 60 ); //12 hours
            await game.connect(accounts[PLAYER_ID1]).claimReward(tokenId0);

            await checkBalance(ethers.utils.parseEther((CLAIM_REWARD * 2).toString()));
        })
    })
})